...
// import named adLibrary + helpers from src/lib/ads.ts
import { adLibrary, getAdForSlot, getInterstitialAdForSlot } from "@/lib/ads";
...
// DEV fallback (same path used in adLibrary) - your toolchain will convert path to URL
const LOCAL_FALLBACK_AD = "/mnt/data/6589e65a-9cdf-4573-a63f-75216cbb5864.png";
...
const HistoryItemComponent = ({ attempt }: { attempt: QuizAttempt }) => {
  ...
  const handleReviewClick = useCallback(
    async () => {
      ...
      try {
        // prefer interstitial config (DB-built)
        const interstitial = await getInterstitialAdForSlot((attempt as any).adSlot);
        if (interstitial) {
          setAdConfig({
            type: interstitial.type === "video" ? "video" : "image",
            url: interstitial.videoUrl ?? interstitial.logoUrl ?? adLibrary.resultsAd.url ?? LOCAL_FALLBACK_AD,
            title: interstitial.videoTitle ?? interstitial.logoHint ?? adLibrary.resultsAd.title,
            duration: interstitial.durationSec ?? Math.floor((interstitial.durationMs ?? (adLibrary.resultsAd.duration ?? 7000)) / 1000),
            skippableAfter: interstitial.skippableAfterSec ?? undefined,
          });
        } else {
          // fallback to DB ad
          ...
          if (dbAd) {
            setAdConfig({
              ...
              url: dbAd.mediaUrl || adLibrary.resultsAd.url || LOCAL_FALLBACK_AD,
              title: dbAd.companyName || adLibrary.resultsAd.title,
              duration: adLibrary.resultsAd.duration ?? 7,
              skippableAfter: adLibrary.resultsAd.skippableAfter ?? 5,
              hint: dbAd.companyName,
            });
          } else {
            // final fallback to in-file adLibrary
            setAdConfig({
              type: (adLibrary.resultsAd.type as "image" | "video") ?? "image",
              url: adLibrary.resultsAd.url || LOCAL_FALLBACK_AD,
              title: adLibrary.resultsAd.title,
              duration: adLibrary.resultsAd.duration ?? 7,
              skippableAfter: adLibrary.resultsAd.skippableAfter ?? 5,
              hint: adLibrary.resultsAd.hint,
            });
          }
        }
      } catch (err) {
        console.error("[HistoryItem] error fetching ad config:", err);
        setAdConfig({
          type: (adLibrary.resultsAd.type as "image" | "video") ?? "image",
          url: adLibrary.resultsAd.url || LOCAL_FALLBACK_AD,
          title: adLibrary.resultsAd.title,
    ...
    [attempt, isDisqualified, isReviewed, isReviewing]
  );
